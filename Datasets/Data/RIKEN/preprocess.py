...
def prepare(force=False):
    if OUTPUT_PATH.exists() and not force:
        ...
        return {"dataset": "RIKEN", "size": len(frame), "invalid_rows": 0}

    raw_frame = pd.read_excel(SOURCE_PATH / "RIKEN_MONA_ALL.xlsx")
    smiles_column = "smiles" if "smiles" in raw_frame.columns else "SMILES"
    ...
    return {"dataset": "RIKEN", "size": len(output_frame), "invalid_rows": invalid_rows}
